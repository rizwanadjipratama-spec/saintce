...
export default function PortalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("[Saintce/Portal] Unhandled error:", error)
  }, [error])

  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-5 text-center">
      <p className="font-mono text-xs uppercase tracking-[0.16em] text-(--signal)">Portal error</p>
      ...
      <div className="flex gap-4">
        ...
        <a href="/portal" className="saintce-button saintce-button--ghost">Dashboard</a>
      </div>
    </div>
  )
}
