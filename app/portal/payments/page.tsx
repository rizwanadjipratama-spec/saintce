...
export default function PortalPaymentsPage() {
  ...
  if (loading) {
    return <p className="text-(--muted)">Loading payment history...</p>
  }

  return (
    <div>
      <div className="border-b border-(--border-soft) pb-8">
        <p className="font-mono text-[0.75rem] uppercase tracking-[0.16em] text-(--signal)">Client portal</p>
        ...
        {payments.length === 0 ? (
          ...
          <div className="space-y-3">
            {payments.map((payment) => (
              <div
                key={payment.id}
                className="saintce-inset rounded-[22px] p-5"
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-1.5 text-sm text-(--muted)">
                      {payment.invoice_number && (
                        <span className="mr-2">{payment.invoice_number} ·</span>
                      )}
                      {payment.service_name && (
                        <span>{payment.service_name}</span>
                      )}
                      {payment.project_name && (
                        <span className="ml-1 text-(--muted)">({payment.project_name})</span>
                      )}
                    </p>
